...
export function getCssModuleLocalIdent(context, _, exportName, options) {
    ...
    const hash = loaderUtils.getHashDigest(Buffer.from(`filePath:${relativePath}#className:${exportName}`), "md5", "base64", 15);
    // Have webpack interpolate the `[folder]` or `[name]` to its real value.
    return loaderUtils.interpolateName(context, hash, options).replace(// Webpack name interpolation returns `about.module_root__2oFM9` for
